...
class Joystick:
    ...
    def check_yaw_setpoint_toggle(self, joy_input, saved_key, magnitude, angle):
        ...
        if self.inputs[joy_input]:
            print("yaw setpoint toggle")
            if magnitude >= 0.95:
                self.saved[saved_key] = angle
                print("set angle to " + str(angle))
            else:
    ...
    def input_yaw_setpoint(self, joy_input_x, joy_input_y, saved_key):
        ...
        if magnitude >= 0.95:
            ...
            print("(input yaw setpoint) set yaw setpoint to " + str(angle) + " with magnitude " + str(magnitude))
    ...
    def set_raw_pwm(self, joy_input_lt, joy_input_rt):
        depth_pwm = int(self.inputs[joy_input_lt] * Joystick.MAX_DEPTH_PWM * 1) - Joystick.MAX_DEPTH_PWM
        yaw_pwm = int(self.inputs[joy_input_rt] * Joystick.MAX_YAW_PWM * -1) + Joystick.MAX_YAW_PWM

        self.publish(Topic.DEPTH_PWM, depth_pwm)
    ...
    def is_depth_valid(self):
        ...
        if not self.depth_state:
            print('(is depth valid) depth was never received')
            return False
        if self.depth_last_received > 100:
            print('(is depth valid) depth not received for 100 ticks')
            return False
        if self.depth_state > Joystick.MAX_DEPTH:
            print('(is depth valid) depth > MAX_DEPTH')
            return False
        return True
    ...
    def input_depth(self, joy_input):
        ...
        if self.inputs[joy_input] > 0.5:
            ...
            self.publish(Topic.DEPTH_PID, False)
            self.publish(Topic.DEPTH_PWM, 0)
            print("(input depth) set depth pwm to 0")
        # If input is -1
            ...
            print("(input depth) set depth pwm to " + str(depth_pwm_input))
        # If input is 0
        else:
            if self.depth_pwm_input != 0:
                ...
                print("(input depth) set depth setpoint to " + str(depth_state))
